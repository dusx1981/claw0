...
# ---------------------------------------------------------------------------
# 1. FailoverReason -- 对 API 调用失败原因进行分类
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2. AuthProfile -- 单个 API key 及其冷却追踪
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4. 简化的 ContextGuard (内联, 来自 s03 模式)
# ---------------------------------------------------------------------------
# 提供 token 估算和消息压缩. 完整的 ContextGuard 在 s03_sessions.py 中;
# 这里只需要 Layer 2 重试洋葱在溢出错误时使用的压缩路径.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. 工具定义 (简化的 bash + read_file, 来自 s02)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. SimulatedFailure -- 让用户触发特定错误类型
# ---------------------------------------------------------------------------
# REPL 命令 /simulate-failure <reason> 设置此标志, 使下次 API 调用
# 抛出模拟错误. 这让用户无需真实故障即可观察三层洋葱如何处理各类失败.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 7. ResilienceRunner -- 三层重试洋葱
# ---------------------------------------------------------------------------
# 这是弹性系统的核心. 它用三层嵌套的重试包裹每次 agent 执行:
#
...
#           这是 s01/s02 教授的内容 -- 运行直到 end_turn 或报错.
#
# 如果所有配置耗尽, 尝试备选模型. 如果全部失败, 抛出 RuntimeError.
# ---------------------------------------------------------------------------
...
class ResilienceRunner:
    ...
    def run(
        ...
        返回 (final_response, updated_messages).
        所有配置和备选模型都耗尽时抛出 RuntimeError.
        """
