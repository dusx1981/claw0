...
全ての送信メッセージは信頼性のある配信キューを経由する。
送信に失敗した場合はバックオフ付きでリトライ。プロセスがクラッシュした場合は
再起動時にディスクをスキャンする。

    Agent Reply / Heartbeat / Cron
              |
        chunk_message()       -- プラットフォーム制限に応じて分割
              |
        DeliveryQueue.enqueue()  -- ディスクに書き込み (write-ahead)
              |
        DeliveryRunner (バックグラウンドスレッド)
              |
...
# ---------------------------------------------------------------------------
# 1. QueuedDelivery -- キューエントリのデータ構造
# ---------------------------------------------------------------------------
...
def compute_backoff_ms(retry_count: int) -> int:
    """ジッター付き指数バックオフ。+/- 20% のジッターでサンダリングハードを回避。"""
    if retry_count <= 0:
        return 0
...
# ---------------------------------------------------------------------------
# 2. DeliveryQueue -- ディスク永続化型の信頼性配信キュー
# ---------------------------------------------------------------------------
# Write-ahead: 先にディスクに書き込んでから配信を試行。
# アトミック書き込み: tmp ファイル + os.replace()、クラッシュセーフ。
...
class DeliveryQueue:
    ...
    def enqueue(self, channel: str, to: str, text: str) -> str:
        """キューエントリを作成しアトミックにディスクに書き込む。delivery_id を返す。"""
    ...
    def fail(self, delivery_id: str, error: str) -> None:
        """retry_count をインクリメントし次のリトライ時刻を計算。上限到達時は failed/ に移動。"""
    ...
    def load_pending(self) -> list[QueuedDelivery]:
        """キューディレクトリをスキャンし、全ペンディングエントリをエンキュー時刻順で返す。"""
    ...
    def retry_failed(self) -> int:
        """全 failed/ エントリを retry_count リセットしてキューに戻す。"""
        count = 0
        if not self.failed_dir.exists():
            return count
        for file_path in self.failed_dir.glob("*.json"):
...
def chunk_message(text: str, channel: str = "default") -> list[str]:
    """メッセージをプラットフォーム適切なチャンクに分割。2段階: 段落分割、次にハードカット。"""
    if not text:
        return []
...
class DeliveryRunner:
    ...
    def start(self) -> None:
        """リカバリスキャンを実行後、バックグラウンド配信スレッドを開始。"""
        self._recovery_scan()
    ...
    def _process_pending(self) -> None:
        """next_retry_at <= now の全ペンディングエントリを処理。"""
        pending = self.queue.load_pending()
        now = time.time()
...
# ---------------------------------------------------------------------------
# 5. MockDeliveryChannel -- シミュレート配信チャネル
# ---------------------------------------------------------------------------
...
class MockDeliveryChannel:
    ...
    def send(self, to: str, text: str) -> None:
        """送信をシミュレート。設定された fail_rate で ConnectionError を送出。"""
...
# ---------------------------------------------------------------------------
# 7. HeartbeatRunner -- 配信キューを通じて出力するタイマー
# ---------------------------------------------------------------------------
...
def handle_repl_command(
    ...
    """REPL コマンドを処理する。処理した場合 True を返す。"""
    if cmd == "/queue":
