"""
Section 04: チャネル -- 「同じ脳、多くの口」

Channelはプラットフォームの違いをカプセル化し、エージェントループは
統一されたInboundMessageだけを扱う。新しいプラットフォームの追加 =
receive() + send()を実装するだけで、ループは変更不要。
...
class InboundMessage:
    """全チャネルがこの形式に正規化する。エージェントループはInboundMessageだけを扱う。"""
    text: str
    sender_id: str
    channel: str = ""
    account_id: str = ""
...
class ChannelAccount:
    """ボットごとの設定。1つのチャネルタイプで複数のボットを実行できる。"""
    channel: str
    account_id: str
    token: str = ""
...
# ---------------------------------------------------------------------------
# TelegramChannel -- Bot APIロングポーリング
# ---------------------------------------------------------------------------
...
class TelegramChannel(Channel):
    ...
    # -- テキスト結合 (1秒ウィンドウ) --
    # Telegramは長い貼り付けを断片に分割するため、バッファして1秒の沈黙後に出力する。
...
class FeishuChannel(Channel):
    ...
    def parse_event(self, payload: dict, token: str = "") -> InboundMessage | None:
        """Feishuイベントコールバックをパースする。検証用の簡易トークンチェック。"""
