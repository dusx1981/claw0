"""
Section 05: ゲートウェイとルーティング -- 「全てのメッセージが宛先を見つける」

ゲートウェイはメッセージのハブ: 全ての受信メッセージは
(agent_id, session_key) に解決される。ルーティングシステムは5階層の
バインディングテーブルで、最も具体的なものから最も一般的なものへと照合する。

    受信メッセージ (channel, account_id, peer_id, text)
           |
    +------v------+     +----------+
    ...
    +------+------+     +----------+
           |
    +------v------+
    |   Routing    |  5階層: peer > guild > account > channel > default
    +------+------+
           |
     (agent_id, session_key)
           |
    +------v------+
    | AgentManager |  エージェントごとの設定 / ワークスペース / セッション
    +------+------+
           |
        LLM API

実行方法:  cd claw0 && python ja/s05_gateway_routing.py
...
# バインディング: 5階層のルート解決
# ---------------------------------------------------------------------------
# 階層 1: peer_id    -- 特定のユーザーをエージェントにルーティング
# 階層 2: guild_id   -- ギルド/サーバーレベル
# 階層 3: account_id -- ボットアカウントレベル
# 階層 4: channel    -- チャネル全体 (例: 全 Telegram)
# 階層 5: default    -- フォールバック
...
class BindingTable:
    ...
    def resolve(self, channel: str = "", account_id: str = "",
        ...
        """階層 1-5 を順に走査し、最初の一致を返す。(agent_id, matched_binding) を返す。"""
...
# ---------------------------------------------------------------------------
# ゲートウェイサーバー (WebSocket, JSON-RPC 2.0)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# デモ: デュアルエージェント (luna + sage) + ルートバインディング
# ---------------------------------------------------------------------------
